...
async def get_dimension_analysis():
    ...
    return {
        ...
        "dimension_performance": [
            {"dimensions": row[0], "avg_execution_time": round(row[1], 2), "count": row[2]} 
    ...
    async with db.get_connection() as conn:
        ...
        for row in data:
            research_data.append({
                ...
                "visualization_requested": bool(row[6]) if row[6] is not None else False
            })
...
async def get_usage_charts():
    ...
    if tool_data:
        tool_fig = go.Figure(data=[go.Pie(
    ...
    return {
        "tool_usage_chart": tool_chart,
        "dimension_chart": dim_chart
    }
