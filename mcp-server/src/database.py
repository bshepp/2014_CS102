...
class DatabaseManager:
    ...
    async def update_tool_stats(self, tool_name: str, execution_time_ms: int, success: bool, session_id: str):
    ...
    async def update_dimension_stats(self, dimensions: int, shape_type: str, execution_time_ms: int):
        ...
        async with self.get_connection() as conn:
            ...
            await conn.execute(query, (dimensions, shape_type, today, execution_time_ms, execution_time_ms))
            await conn.commit()
    ...
    async def _get_usage_stats_sqlite(self, days: int = 30) -> Dict[str, Any]:
        ...
        async with self.get_connection() as conn:
            ...
            return {
                "tool_usage": [dict(zip([col[0] for col in cursor.description], row)) for row in tool_stats],
                "dimension_popularity": [dict(zip([col[0] for col in cursor.description], row)) for row in dim_stats],
                "error_rates": [dict(zip([col[0] for col in cursor.description], row)) for row in error_stats],
            }
