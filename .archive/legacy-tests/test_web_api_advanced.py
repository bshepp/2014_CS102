...
def test_ellipsoid_endpoint():
    ...
    test_cases = [
        {"dimensions": 2, "semi_axes": [3.0, 4.0], "expected_type": "HyperEllipsoid"},
        {
            "dimensions": 3,
    ...
    for case in test_cases:
        try:
            ...
            if response.status_code == 200:
                data = response.json()
                print(
                    f"✅ {case['dimensions']}D ellipsoid (axes={case['semi_axes']}): {data['shape_type']}"
                )
                print(f"   - Volume: {data['volume']:.6f}")
                print(f"   - Surface Area: {data['surface_area']:.6f}")
                print(
                    f"   - Axis ratio: {data['additional_properties']['axis_ratio']:.6f}"
                )
                ...
                if case["expected_type"] == data["shape_type"]:
                    ...
                    print(
                        f"   ❌ Shape type mismatch: {data['shape_type']} vs {case['expected_type']}"
                    )

            else:
                print(
                    f"❌ {case['dimensions']}D ellipsoid failed: {response.status_code}"
                )
    ...
    for case in test_cases:
        try:
            ...
            if response.status_code == 200:
                data = response.json()
                print(
                    f"✅ {case['dimensions']}D simplex (side={case['side_length']}): {data['shape_type']}"
                )
                print(f"   - Volume: {data['volume']:.6f}")
                print(f"   - Surface Area: {data['surface_area']:.6f}")
                ...
                print(
                    f"   - Circumradius: {data['additional_properties']['circumradius']:.6f}"
                )
                ...
                print(
                    f"❌ {case['dimensions']}D simplex failed: {response.status_code}"
                )
                print(f"   Error: {response.text}")
        except Exception as e:
...
def test_pyramid_endpoint():
    ...
    test_cases = [
        ...
        },
        {
            ...
            "height": 3.0,
            "expected_type": "Square Pyramid",
        },
        {
            "dimensions": 4,
            "base_side_length": 2.5,
            "height": 4.0,
            "expected_type": "4D HyperPyramid",
        },
    ]
    ...
    for case in test_cases:
        try:
            ...
            if response.status_code == 200:
                data = response.json()
                print(
                    f"✅ {case['dimensions']}D pyramid (base={case['base_side_length']}, height={case['height']}): {data['shape_type']}"
                )
                print(f"   - Volume: {data['volume']:.6f}")
                print(f"   - Surface Area: {data['surface_area']:.6f}")
                print(f"   - Vertices: {data['additional_properties']['vertices']}")
                print(f"   - Edges: {data['additional_properties']['edges']}")
                print(
                    f"   - Slant height: {data['additional_properties']['slant_height']:.6f}"
                )
                print(
                    f"   - Base volume: {data['additional_properties']['base_volume']:.6f}"
                )
                print(f"   - Formula: {data['volume_formula']}")
                ...
                print(
                    f"❌ {case['dimensions']}D pyramid failed: {response.status_code}"
                )
                print(f"   Error: {response.text}")
        except Exception as e:
    ...
    for case in error_cases:
        try:
            response = requests.post(
                f"{API_BASE}/{case['endpoint']}", json=case["data"]
            )
            if response.status_code == 400:
                ...
                print(
                    f"❌ {case['endpoint']} should have failed but got: {response.status_code}"
                )
        except Exception as e:
...
def test_mathematical_accuracy():
    ...
    try:
        ...
        response = requests.post(
            f"{API_BASE}/ellipsoid", json={"dimensions": 2, "semi_axes": [3.0, 4.0]}
        )
        if response.status_code == 200:
            data = response.json()
            ...
            if abs(actual_volume - expected_volume) < 0.001:
                ...
                print(
                    f"❌ 2D ellipse volume inaccurate: {actual_volume:.6f} vs {expected_volume:.6f}"
                )

        # Test 3D tetrahedron (should be (√2/12) * s³)
        response = requests.post(
            f"{API_BASE}/simplex", json={"dimensions": 3, "side_length": 2.0}
        )
        if response.status_code == 200:
            data = response.json()
            ...
            if abs(actual_volume - expected_volume) < 0.001:
                ...
                print(
                    f"❌ 3D tetrahedron volume inaccurate: {actual_volume:.6f} vs {expected_volume:.6f}"
                )

        # Test 3D square pyramid (should be (1/3) * base² * height)
        response = requests.post(
            f"{API_BASE}/pyramid",
            json={"dimensions": 3, "base_side_length": 4.0, "height": 3.0},
        )
        if response.status_code == 200:
            data = response.json()
            ...
            if abs(actual_volume - expected_volume) < 0.001:
                ...
                print(
                    f"❌ 3D square pyramid volume inaccurate: {actual_volume:.6f} vs {expected_volume:.6f}"
                )
...
def main():
    ...
    if passed >= total - 1:  # Allow for one minor failure
        ...
        print("        -H 'Content-Type: application/json' \\")
        print(
            '        -d \'{"dimensions": 3, "base_side_length": 2.0, "height": 3.0}\''
        )
