...
def test_hyperellipsoid_basic():
    ...
    try:
        ...
        if abs(actual_volume - expected_volume) < 1e-10:
            ...
            print(f"❌ 2D ellipse volume incorrect: {actual_volume:.6f} vs {expected_volume:.6f}")
        ...
        if abs(eccentricity - expected_eccentricity) < 1e-10:
            ...
            print(f"❌ 2D ellipse eccentricity incorrect: {eccentricity:.6f} vs {expected_eccentricity:.6f}")
        ...
        if abs(actual_volume - expected_volume) < 1e-10:
            ...
            print(f"❌ 3D ellipsoid volume incorrect: {actual_volume:.6f} vs {expected_volume:.6f}")
        ...
        if abs(axis_ratio - expected_ratio) < 1e-10:
            ...
            print(f"❌ 3D ellipsoid axis ratio incorrect: {axis_ratio:.6f} vs {expected_ratio:.6f}")
...
def test_hyperellipsoid_dimensions():
    ...
    try:
        ...
        test_cases = [
            ...
            (3, [1.0, 2.0, 3.0], (4/3) * math.pi * 1.0 * 2.0 * 3.0),  # 3D: (4/3)π * a * b * c
