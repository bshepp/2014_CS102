...
from geometry_engine import (
    NDShape, HyperSphere, HyperCube, HyperEllipsoid, Simplex, HyperPyramid,
    OriginalSphere, GeometryAgent, JavaBridge
)
...
class TestHyperSphere:
    ...
    def test_hypersphere_surface_area(self, sample_spheres, tolerance):
        ...
        assert abs(sample_spheres["unit_2d"].get_surface_area() - expected_2d) < tolerance
        
        # 3D sphere: 4 * π * r²
        expected_3d = 4 * math.pi * 1.0**2
        assert abs(sample_spheres["unit_3d"].get_surface_area() - expected_3d) < tolerance
    ...
    def test_hypersphere_scaling(self):
        ...
        assert abs(sphere2.get_surface_area() / sphere1.get_surface_area() - 4.0) < 1e-10
...
class TestJavaBridge:
    ...
    @pytest.mark.skipif(not os.path.exists("Sphere.java"), reason="Sphere.java not found")
    ...
    @pytest.mark.skipif(not os.path.exists("MultiSphere.java"), reason="MultiSphere.java not found")
