    ...
    for case in test_cases:
        try:
            ...
            if response.status_code == 200:
                ...
                print(f"   - Analysis: {data['analysis']['mathematical_properties']['is_regular']}")
            else:
                print(f"❌ {case['dimensions']}D {case['shape_type']} tiling failed: {response.status_code}")
...
def test_hexagonal_tiling_endpoint():
    ...
    for case in test_cases:
        try:
            ...
            if response.status_code == 200:
                data = response.json()
                print(f"✅ Hexagonal tiling (side={case['side_length']}, density={case['density']}):")
                print(f"   - Tiles: {data['tile_count']}")
                print(f"   - Coverage: {data['coverage_efficiency']:.3f}")
                print(f"   - Pattern: {data['pattern_properties']['pattern_type']}")
                print(f"   - Coordination: {data['analysis']['mathematical_properties']['coordination_number']}")
            else:
    ...
    for i, case in enumerate(test_cases):
        try:
            ...
            if response.status_code == 200:
                ...
                print(f"   - Coverage: {data['coverage_efficiency']:.3f}")
                print(f"   - Pattern: {data['pattern_properties']['pattern_type']}")
                print(f"   - Regular: {data['analysis']['mathematical_properties']['is_regular']}")
            else:
...
def test_health_endpoint():
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            tiling_features = [f for f in data['features'] if 'tiling' in f.lower() or 'tessellation' in f.lower() or 'voronoi' in f.lower()]
...
def main():
    ...
    if passed >= total - 1:  # Allow for one minor failure
        ...
        print("        -d '{\"tiling_type\": \"hexagonal\", \"dimensions\": 2, \"bounds\": [[0, 10], [0, 10]], \"side_length\": 1.0}'")
        
        print("\n   # Create Voronoi diagram")
        print("   curl -X POST http://localhost:8000/api/tiling \\")
        print("        -H 'Content-Type: application/json' \\")
        print("        -d '{\"tiling_type\": \"voronoi\", \"dimensions\": 2, \"bounds\": [[0, 10], [0, 10]], \"num_random_seeds\": 8}'")
